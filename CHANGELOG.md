...
- **原子合成**：修复排布阶段点击「合成」无效果的问题。`updateState` 需在回调内直接修改传入的 state，合成逻辑改为原地修改 `s.pools` 后生效。

### 优化

- **连通子集选择 UI**：选择按钮改为中性灰色样式，不再使用琥珀/青等子集颜色；连通子集在棋盘上的标记改为仅描边环（不填满），不遮挡原子。
...
- **战局信息栏**：右侧新增可滑动查看的「战局信息」面板，记录双方放置、撤回、发动效果、进攻、破坏等，主游戏与 AI 对战均适用；支持自动滚到底部。
- **AI 多次进攻日志**：AI 一回合内多次进攻时，信息栏按每次进攻分别记录伤害与破坏，不丢条；重复播报去重，汇总提示「共 N 次进攻，造成 X 点伤害」。
- **放置播报以实际为准**：批量放置时，信息栏以格内实际成功放置的原子数为准（含严格模式下去重汇总），单次放置仍为 1。
- **信息栏展示优化**：每回合用红色虚线隔开（「Px 开始排布/开始行动」前）；重要信息如「造成 N 点伤害」「被破坏/破坏 N 个」中的数字用红色高亮。
- **AI 对战关卡设置**：关卡设置界面将「开始游戏」按钮置于右侧，与配置同排，缩短纵向高度。
...
- **主游戏双人**：主游戏（主菜单进入）默认均为双人对战，P0 与 P1 均由玩家操控；返回主菜单后再次开始也默认「双人」模式，需主动选择「进入 AI 对战」才会使用 AI（P1）。
- **进攻确认**：双人模式下 P1 选择己方格子与对方格子后，右侧显示「确认进攻？」与确认/取消按钮，由玩家操作；仅 AI 对战且轮到 P1（AI）时显示「AI 进攻中…」。
- **伤害数值动画**：进攻造成对方掉血时，在被进攻的格子附近弹出红色数值动画（如 -1），向上飘动并淡出，玩家进攻 AI、AI 进攻玩家、直接攻击均会显示。
- **破坏原子虚化**：AI 进攻或红效果导致玩家原子被破坏时，被破坏的原子播放虚化动画（缩小、变暗、淡出），与玩家进攻时的破坏动画一致。
