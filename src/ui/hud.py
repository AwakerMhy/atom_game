...
RULES_OVERLAY_LINES = [
    ...
    "阶段1：点击「多抽原子」「多放置」「多进攻」之一。阶段2：拖动原子到己方格放置，点「结束排布」。",
    "阶段3：点击己方格子进攻再点对方格；或点击己方红/蓝/绿原子发动效果；点「结束回合」。",
    "攻击力=己格黑原子竖向跨度；防御力=对方格黑原子横向跨度。攻>防可破坏黑原子。",
    "红=对方选 y 个黑原子破坏；蓝=己方得 y 黑；绿=己方回 y 血。y=该原子相邻黑原子数。",
]
...
def draw_phase_3_prompt(
    ...
    effect_rule = "红=破坏y个对方黑原子 蓝=己方得y个黑 绿=己方回y血(y=该原子相邻黑数)。"
    if action_substate == "idle":
        ...
        text = "红效果仅破坏黑原子：点击对方黑原子选中（红高亮），再点可撤销；选够后点「确认」"
    else:
