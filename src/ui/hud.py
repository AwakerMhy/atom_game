...
RULES_OVERLAY_LINES = [
    ...
    "1. 目标：将对方生命降至 0。每方 3 格，格点放原子（黑/红/蓝/绿）。",
    "2. 游戏阶段：",
    "阶段1：拖动原子到己方格放置，点「结束排布」。",
    "阶段2：点击己方红/蓝/绿原子发动效果。",
    "阶段3：点击己方格子进攻再点对方格；随后「结束回合」。",
    "3. 场地和进攻规则：",
    "三角形网格的边长为1",
    "每回合己方「有黑原子的格子」各可以发动一次进攻",
    "格子的攻击力为格子中黑原子竖向跨度除以sqrt(3)/2；",
    "格子的防御力为格子中黑原子横向跨度。",
    "攻>防可破坏一个黑原子，并对玩家造成1点伤害。",
    "若对方格子没有黑原子，则直接造成发动攻击的格子的攻击力数值伤害。",
    "",
    "每个格子中，黑原子只能随机排布，其他原子可以选择在已有黑原子的邻居位置排布"
    "黑原子之外的原子的效果取决于它们所连接的黑原子数目：",
    "4. 原子持续性效果",
    "红：进攻时己方格红原子连接黑原子数目为 x，则增加可以破坏的原子数目x。",
    "蓝：遭到进攻时己方格蓝原子连接黑原子数目为 x，则降低可以破坏的原子数目x（最低为0）。",
    "绿：回合结束时获得「己方所有绿原子邻接黑原子数」之和的黑原子。",
    "5. 原子的点击效果",
    "红：发动效果时对方须破坏 y 个黑原子；",
    "蓝：与该蓝相邻的黑原子下一回合内不可被破坏，期间以蓝色高亮。",
    "绿：该绿原子就地变为一个黑原子。",
]
